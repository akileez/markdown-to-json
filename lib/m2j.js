...
function processFile(filename, width)
{
    ...
    if (_metadata)
    {
        ...
        _metadata.basePath = path.dirname(filename) + '/' + filename.split( path.sep ).slice( -1 )[0];
        _metadata.baseDirFileExt = path.dirname(filename).split( path.sep ).slice( -1 )[0] + '/' + path.basename(filename);
        _metadata.baseDir  = path.dirname(filename).split( path.sep ).slice( -1 )[0];
        _metadata.baseFileExt = path.basename(filename);
        _metadata.baseFile = path.basename(filename, path.extname(filename));
        _metadata.baseExt  = path.extname(filename);
    }
